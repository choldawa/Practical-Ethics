...
function trialStart() {
	...
	generateDots(fullSet, currDataset[initialSliderVal].g1, 1);
	generateDots(fullSet, currDataset[initialSliderVal].g2, 2);
	generateDots(fullSet, currDataset[initialSliderVal].g3, 3);
	generateDots(fullSet, currDataset[initialSliderVal].g4, 4);
...
function trialDone() {
	$(document).ready(function () {
		if (trialNumber == attentionTrial || catchTrials.includes(trialNumber)) {
			att = 1;
		} else {
			att = 0;
		}
	...
	console.log("IS ATT, TrialNumber", att, trialNumber);
